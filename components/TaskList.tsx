...
const TaskItem: React.FC<TaskItemProps> = React.memo(({ 
    ...
    <>
      ...
      <div className="flex-1 p-3.5 flex flex-col min-w-0 gap-1.5">
        <div className="flex items-center justify-between gap-2">
          ...
          {isDragEnabled && (
            <div onPointerDown={(e) => controls.start(e)} className="text-slate-400 dark:text-slate-600 cursor-grab active:cursor-grabbing p-2 touch-none -mr-2">
  ...
  return isDragEnabled ? (
    // Fix: Using as any to bypass TypeScript error on dragListener/dragControls props which are valid for Reorder.Item in this context
    <Reorder.Item {...({ value: task, id: task.id, dragListener: false, dragControls: controls } as any)} className={containerClasses} onClick={() => onSelect(task)}>
      {Content}
    </Reorder.Item>
  ) : (
...
export const TaskList: React.FC<TaskListProps> = ({ tasks, setTasks, selectedTaskId, onSelectTask, onAddNewTask, onGoHome }) => {
  ...
  return (
    <div className="h-full flex flex-col bg-slate-50 dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 w-full md:w-80 flex-shrink-0">
      <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-white dark:bg-slate-900">
        <div className="flex items-center gap-3">
          <h2 className="text-xl font-bold tracking-tighter uppercase bg-clip-text text-transparent bg-gradient-to-br from-indigo-500 to-blue-600">TaskFlow</h2>
        </div>
      ...
      <div className="p-4 space-y-4">
        <div className="relative">
          ...
          <input 
            ...
            className="w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-md pl-9 pr-3 py-2 text-xs font-semibold focus:border-indigo-500 outline-none text-slate-700 dark:text-slate-200"
          />
        </div>
        ...
        {showFilters && (
          ...
          <motion.div 
            {...({ initial: { height: 0, opacity: 0 }, animate: { height: 'auto', opacity: 1 } } as any)}
            className="space-y-3 pt-2 border-t border-slate-100 dark:border-slate-800 overflow-hidden"
          >
            <div className="flex flex-col gap-1.5">
                <label className="text-[9px] font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400">Status</label>
                <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="w-full bg-white dark:bg-slate-800 border border-slate-200 rounded-md px-2 py-1.5 text-[11px] font-semibold">
