...
const MetadataCard: React.FC<{ icon: React.ElementType; label: string; children: React.ReactNode; onClick?: () => void; iconColor?: string }> = ({ 
  ...
  <div onClick={onClick} className={`flex items-center gap-3 p-3.5 rounded-xl bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 transition-all ${onClick ? 'cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800 active:scale-95 hover:border-indigo-200 dark:hover:border-indigo-900' : ''}`}>
    ...
    <div className="min-w-0">
      ...
      <div className="text-[13px] font-bold text-slate-800 dark:text-slate-100 truncate leading-none">{children}</div>
    </div>
  </div>
);
  ...
  if (!task) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-950 text-slate-400 select-none">
        ...
        <p className="text-[10px] font-semibold uppercase tracking-widest opacity-50">Choose from the list to begin</p>
      </div>
    );
  }
      ...
      <div className="w-full px-4 md:px-5 lg:px-6 py-6 space-y-5">
        ...
        <div className="flex flex-col md:flex-row items-start justify-between gap-4">
          <div className="space-y-2 flex-1 min-w-0">
            <div className="flex items-center gap-2">
              ...
              {isUrgent && (
                <span className="flex items-center gap-1 px-2 py-0.5 bg-red-600 text-white text-[10px] font-bold uppercase tracking-wider rounded shadow-sm shadow-red-500/20">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            {task.status !== 'done' && (
              <button onClick={() => onStatusChange(task.id, 'done')} className="px-4 py-2 text-xs font-bold text-white bg-emerald-600 dark:bg-emerald-600 rounded-lg hover:bg-emerald-500 transition-all shadow-md shadow-emerald-500/20 active:scale-95 uppercase tracking-widest">
                Mark Done
              </button>
            )}
          ...
          {Object.keys(STATUS_CONFIG).map((key) => {
            ...
            return (
              <button
                ...
                className={`px-4 py-2 text-[10px] font-semibold rounded-lg transition-all uppercase tracking-wider whitespace-nowrap ${isActive ? `${config.color} ${config.text} shadow-md` : 'text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800'}`}
              >
                {config.label}
              </button>
            );
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
          ...
          <MetadataCard icon={Flag} label="Priority" onClick={() => onPriorityChange(task.id, isUrgent ? 'not-urgent' : 'urgent')} iconColor={isUrgent ? 'text-red-600 dark:text-red-400' : 'text-slate-500'}>
            <span className={isUrgent ? 'text-red-600 dark:text-red-400 font-semibold' : 'font-semibold'}>{isUrgent ? 'Urgent' : 'Normal'}</span>
          </MetadataCard>
        ...
        <div className="bg-white dark:bg-slate-900/30 p-5 md:p-6 rounded-xl border border-slate-200 dark:border-slate-800/60 shadow-sm">
          <div className="text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2.5">Task Overview</div>
